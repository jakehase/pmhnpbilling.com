# Blog Performance Tracker

## Overview
Tracks performance of blog posts to inform future content strategy.

## Scoring System
- **High Performer**: >100 impressions/week OR >5 clicks/week OR avg position <15
- **Medium Performer**: 20-100 impressions/week OR 1-5 clicks/week OR avg position 15-30
- **Low Performer**: <20 impressions/week OR 0 clicks/week OR avg position >30

## Post Performance Log

### 2026-02-01 - Essential CPT Codes for PMHNPs: The Complete 2026 Billing Guide
- **URL**: /blog/essential-cpt-codes-pmhnp-guide.html
- **Target Keywords**: cpt codes pmhnp, psychiatric billing codes, 90791 90792 billing, psychotherapy cpt codes, medication management billing
- **Status**: 🟡 Not yet indexed
- **Week 1**: Pending...
- **Notes**: Fills keyword gap - CPT codes not previously covered. High-intent topic with practical tables and reimbursement data.

### 2026-01-31 - Telehealth Billing for Illinois PMHNPs: 2026 Compliance Guide
- **URL**: /blog/illinois-telehealth-billing-guide.html
- **Target Keywords**: illinois telehealth billing, pmhnp telehealth, telehealth reimbursement 2026
- **Status**: 🟡 Not yet indexed
- **Week 1**: Pending...
- **Week 2**: Pending...
- **Notes**: 

### 2026-01-30 - Navigating the Prior Authorization Maze
- **URL**: /blog/prior-authorization-guide.html
- **Target Keywords**: prior authorization pmhnp, chicago prior auth, psychiatric medication authorization
- **Status**: 🟡 Not yet indexed
- **Week 1**: Pending...
- **Notes**:

### 2026-01-30 - Mental Health Parity in Illinois
- **URL**: /blog/mental-health-parity-illinois.html
- **Target Keywords**: mental health parity illinois, pmhnp insurance compliance, parity violations
- **Status**: 🟡 Not yet indexed
- **Week 1**: Pending...
- **Notes**:

### 2026-01-30 - The $50,000 Mistake: Common Billing Errors
- **URL**: /blog/costly-billing-mistakes.html
- **Target Keywords**: pmhnp billing mistakes, psychiatric billing errors, revenue loss
- **Status**: 🟡 Not yet indexed
- **Week 1**: Pending...
- **Notes**:

## Topic Performance Summary

### High Performing Topics (Double Down)
_None yet - waiting for indexing data_

### Medium Performing Topics (Optimize)
_None yet - waiting for indexing data_

### Low Performing Topics (Avoid/Improve)
_None yet - waiting for indexing data_

## Content Strategy Insights

### What's Working
- 

### What's Not Working
- 

### Keyword Gaps to Fill
- 

## Next Content Recommendations

Based on performance data:
1. 
2. 
3. 

---
*Last Updated*: 2026-02-01
*Next Review*: 2026-02-08 (after SEO index check)
